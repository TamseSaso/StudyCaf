  ...
  <div class="Body w-full p-0 md:p-[25px] bg-black rounded-[70px] flex flex-col justify-start items-center">
    ...
    <div class="Register w-full px-4 md:px-20 py-10 md:py-[35px] bg-[#f9d4b3] md:rounded-[70px] rounded-[32px] flex flex-col md:flex-row justify-between items-center gap-10">
        <div class="ChangePasswordForm w-full max-w-md flex flex-col justify-start items-start gap-5">
            ...
            <form action="update_password.php" method="POST" class="flex flex-col gap-4 w-full">
                <input type="hidden" name="token" value="<?php echo $_GET['token']; ?>" />

                <div class="LightFieldDefault w-full relative">
                    <input name="new_password" type="password" class="Rectangle w-full h-12 pl-6 bg-white rounded-3xl" placeholder="TYPE YOUR NEW PASSWORD" required>
                </div>

                <div class="LightFieldDefault w-full relative">
                    <input name="confirm_password" type="password" class="Rectangle w-full h-12 pl-6 bg-white rounded-3xl" placeholder="CONFIRM NEW PASSWORD" required>
                </div>
